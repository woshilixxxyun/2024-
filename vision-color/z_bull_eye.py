...
def bull(img):
    data=[None]*4
    imgCopy=img.copy()
    #img_cut = imgCopy[60:340, 170:460]#裁切区域
    gray_img = cv2.cvtColor(imgCopy,cv2.COLOR_BGR2GRAY)

    #thresh=cv2.getTrackbarPos("thresh","Thresh")
    _,th_img = cv2.threshold(gray_img,154,255,cv2.THRESH_BINARY)#二值化
    blur_img = cv2.medianBlur(th_img,1)#模糊
    ...
    if circles is not None:
        circles = circles[0]#只取第一个
        #圆心坐标为（i[0],i[1])，此处可能需要强制转换为整数
        for i in circles:
            cv2.circle(imgCopy, (int(i[0]), int(i[1])), 50, (255, 0, 0), -1)
        #print(int(i[0]),int(i[1]))
        cv2.imshow('imgCopy',imgCopy)
        #测出相对中点
        xm=145
        ym=133
        cv2.circle(img_cut, (xm, ym), 2, (255, 225, 0), -1)#绘制
